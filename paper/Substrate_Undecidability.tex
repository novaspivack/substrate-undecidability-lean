% Substrate Undecidability — LaTeX source (Lean 4 / Mathlib companion library: substrate-undecidability-lean)
...
\title{Substrate Undecidability\\[0.2em]
  {\large Why No Internal Observer Can Decide Whether Reality Is Discrete or Continuous,\\[0.1em]
  and What Can Adjudicate Instead}}
\author{Nova Spivack\\
...
(\cref{sec:glimmer}), whose classification is machine-checked, leaving the Born-martingale
physical premise and a second, distinct framework-transfer premise open (\cref{sec:open}).
...
(\cref{sec:glimmer}), whose definitions and separations are machine-checked but whose two
remaining bridge premises are disclosed as open, not folded in as an established result. Axis~B\('\)
...
independently of that Lean status, are \textbf{two} distinct premises. The \textbf{Born-martingale
lemma}: that the actual physical Born measure, as complete internal semantics per NEMS
Papers~13/14 \citep{SpivackP13,SpivackP14}, admits no successful computable martingale on the
realized transcript, given the non-degeneracy premise of NEMS Paper~12 \citep{SpivackP12}. The
\textbf{framework-transfer premise}: that the degree bound of UGP Paper~51 \citep{SpivackUGP51}
--- proved there for the GTE adjudication layer under that paper's own named premise bundle, not
for an arbitrary framework's transcripts --- transfers to this paper's framework-agnostic
\texttt{World} model. We state the dependency chain honestly in
...
martingale fact) is machine-checked; the two physical/interpretive premises connecting it to the
actual Born measure and to UGP Paper~51's degree bound are not.
...
relative to \(E\) \emph{conditionally} on a second premise, disclosed in full at
derivation-chain step~3 below with the same honesty as the Born-martingale lemma (step~2): UGP
Paper~51's bound \citep{SpivackUGP51} proves \(f \le_T \mathbf{0}'\) for the GTE adjudication
layer specifically, under that paper's own named premise bundle (the Arithmetic Self-Reference
record theory and its \(D_1\)--\(D_5\) DSAC/\([D]\)-adjudicator constraints), not as a proved
consequence for an arbitrary framework's transcripts. Ruling out the residual fifth case ---
non-computable, not computably random, and not \(\Delta^0_2\) --- for \emph{this} paper's
framework-agnostic \texttt{World} model therefore requires importing that bound across a
framework boundary; step~3 names this precisely and discloses it as open.
...
3. Truly random & \(f\) computably random relative to \(\mu\), not \(\Delta^0_2\) & a.e.\ Martin-L\"of-random sequence & excluded by UGP Paper~51 \citep{SpivackUGP51}, conditional on the framework-transfer premise (derivation chain, step~3) \\
\bottomrule
\end{tabular}
\end{center}
...
\item \textbf{UGP Paper~51 (adjudication at degree exactly \(\mathbf{0}'\)) --- and the
framework-transfer premise (open).} \(f \le_T \mathbf{0}'\) \(\Rightarrow\) \(f \notin\)
class~3. UGP Paper~51 \citep{SpivackUGP51} proves this bound for the adjudication layer of the
GTE physical framework specifically: its own premise bundle (named PR1--PR5 there) is stated
over that framework's Arithmetic Self-Reference record theory --- finite PSC-admissible winding
sectors, the \(D_1\)--\(D_5\) constraints on a DSAC/\([D]\)-class adjudicator --- and that paper
states explicitly that its theorems are ``the GTE-specific physical realization'' of a more
general, framework-independent claim it attributes to the NEMS formal theory of transputation,
not a proof of that general claim itself. This paper's \texttt{World} structure
(\cref{def:world}) is deliberately framework-agnostic (an arbitrary state type, step function,
and finite readout; see also the modeling note below) and carries none of that record-theoretic
structure, so using \(f \le_T \mathbf{0}'\) for \emph{this} paper's adjudication transcripts is
not a proved consequence of \citep{SpivackUGP51} but a further, currently undischarged
\textbf{framework-transfer premise}: that whatever process generates a genuinely
complete-semantics transcript in this framework instantiates UGP Paper~51's DSAC/\([D]\)-class
adjudicator model (or the more general abstract theory that paper cites but does not itself
prove). No counterexample is known and we are not aware of an argument that the premise fails,
but it is logically independent of the Born-martingale lemma above, it is not established by
\citep{SpivackUGP51}, and the class-3 exclusion here, and the four-class exhaustiveness claim
stated at the start of this subsection, should both be read as conditional on it.
\item Hence, conditionally on step~2's premise and step~3's framework-transfer premise,
\(f \in\) class~2 (glimmer).
\end{enumerate}
...
Lean side of the ledger are the Born-martingale lemma's physical completeness premise and the
framework-transfer premise for UGP Paper~51's degree bound (both \cref{sec:glimmer}, genuinely
open, not a formalization gap) and the \(\varepsilon\)-tolerant
...
\cref{thm:mdl-uniqueness}'s conclusion is not correctly statable without it. Two honest gaps
remain, and we disclose both precisely rather than rounding them up to a claimed result: the
...
programme; and the framework-transfer premise --- that UGP Paper~51's degree-\(\mathbf{0}'\)
bound, proved there for the GTE adjudication layer under that paper's own premise bundle,
transfers to this paper's framework-agnostic \texttt{World} model --- is likewise not proved
anywhere. Those are the \emph{only} gaps: the Lean module proving the classification's
...
Born-martingale lemma, together with the framework-transfer premise for UGP Paper~51's degree
bound, are the two results standing between this paper's programme and a
genuine resolution of axis~C's relationship to axis~B, which is why we name them explicitly as
...
\item \textbf{The glimmer corollary's two open premises} (\cref{sec:glimmer}) --- the
definitions, four-class taxonomy, and derivation chain of the glimmer corollary are settled
Part~II content (\cref{sec:glimmer}), not an open problem; what remains open are two distinct
physical/interpretive premises. The \textbf{Born-martingale lemma}: that the actual physical
Born measure, as complete internal semantics per NEMS Papers~13/14
\citep{SpivackP13,SpivackP14}, admits no successful computable martingale on the realized
transcript. This is not a randomness-theory gap --- the martingale and computable-randomness
machinery the lemma needs is already fully built and machine-checked (\cref{sec:glimmer}) ---
and it cannot be closed by definitional fiat without smuggling the conclusion. The
\textbf{framework-transfer premise}: that UGP Paper~51's degree-\(\mathbf{0}'\) bound
\citep{SpivackUGP51} --- proved there for the GTE adjudication layer under that paper's own
named premise bundle, which is stated over that framework's specific record theory and not for
an arbitrary framework's transcripts --- transfers to this paper's framework-agnostic
\texttt{World} model, which shares none of that bundle's record-theoretic structure. Neither
premise is discharged by this paper's own machinery, and they are logically independent of one
another; see \cref{sec:glimmer} for the full derivation chain and precisely why these two
premises, and only these two, remain open.
...
Born-martingale lemma's physical completeness premise, the framework-transfer premise
connecting UGP Paper~51's degree-\(\mathbf{0}'\) bound to this paper's framework-agnostic
model, and the question of which substrate actually obtains, are left, honestly, as open.
...
\S\ref{sec:glimmer} framework-transfer premise (UGP Paper~51's degree-\(\mathbf{0}'\) bound applied to this paper's transcripts) & \textbf{Open} --- not proved here or anywhere in the programme at time of writing, and logically independent of the Born-martingale lemma (row above). UGP Paper~51 \citep{SpivackUGP51} proves \(f \le_T \mathbf{0}'\) for the GTE adjudication layer specifically, under that paper's own named premise bundle (its Arithmetic Self-Reference record theory and \(D_1\)--\(D_5\) DSAC/\([D]\)-adjudicator constraints), and that paper itself attributes the framework-independent form of the claim to a separate work (the NEMS formal theory of transputation), not to its own theorems. This paper's \texttt{World} structure (\cref{def:world}) is deliberately framework-agnostic and carries none of that record-theoretic structure, so importing the bound for \emph{this} paper's transcripts is a further, undischarged premise, not a proved consequence of \citep{SpivackUGP51} \\
